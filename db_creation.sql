...
CREATE  TABLE IF NOT EXISTS `vlab`.`exercise` (
  `id` INT NOT NULL COMMENT 'ID of the exercise' ,
  `content` VARCHAR(45) NULL COMMENT 'Actual Content' ,
  `creation_date` TIMESTAMP NULL COMMENT 'Timestamp of first creation' ,
  `mark` VARCHAR(45) NULL COMMENT 'Mark of the evaluated exercise' ,
  `user_id` VARCHAR(255) CHARACTER SET 'utf8' COLLATE 'utf8_bin' NOT NULL COMMENT 'Creator of the exercise' ,
  `status` INT NULL COMMENT 'Draft or Sumbitted or Under Review etc' ,
  `comments` MEDIUMTEXT NULL COMMENT 'Accompanying text' ,
